<x-guest-layout>
    <form method="POST" action="{{ route('register') }}" enctype="multipart/form-data">
        ...
        <div class="mt-4">
            ...
            <select name="role" id="role" class="block mt-1 w-full rounded-md shadow-sm" required>
                <option value="">-- Select Role --</option>
                <option value="SUPERADMIN" {{ old('role') == 'SUPERADMIN' ? 'selected' : '' }}>SUPERADMIN</option>
                <option value="PPIC" {{ old('role') == 'PPIC' ? 'selected' : '' }}>PPIC</option>
                <option value="PROD" {{ old('role') == 'PROD' ? 'selected' : '' }}>PROD</option>
                <option value="GUD" {{ old('role') == 'GUD' ? 'selected' : '' }}>GUD</option>
            </select>
